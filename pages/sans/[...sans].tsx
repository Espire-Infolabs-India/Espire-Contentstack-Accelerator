...
const SansPage: NextPage<PageProps> = ({
  page,
  pageUrl,
  header,
  footer,
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths: { params: { sans: string[] } }[] = [];
  entryPaths.forEach((entry) => {
    if (entry?.url && entry?.url.startsWith("/sans/")) {
      const slug = entry?.url.replace("/sans/", "");
      if (slug) {
        paths.push({
          params: {
            sans: slug.split("/"),
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
  try {
    ...
    if (!params || !params["sans"]) {
      return { notFound: true };
    }

    const slugParts = params["sans"];
    const pageUrl = `/sans/${
      Array.isArray(slugParts) ? slugParts.join("/") : slugParts
    }`;

    const res: Page = await getPageRes(pageUrl, "_sans", locale, getSiteName());
